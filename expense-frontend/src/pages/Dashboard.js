...
const defaultSplitForm = { person: "", amount: "", direction: "owes_me", note: "" };
const defaultBudgetDraft = { category: "Food", amount: "" };

const parseVoiceExpense = (text) => {
  const numberMatch = text.match(/(\d+(\.\d+)?)/);
  const amount = numberMatch ? numberMatch[1] : "";
  const category = suggestCategory(text, text);
  let description = text
    .replace(/\b(add|please|spent|spend|rupees|rs|for|on|today|the|an?)\b/gi, "")
    .replace(/\d+(\.\d+)?/g, "")
    .replace(/\s+/g, " ")
    .trim();
...
const preprocessReceiptImage = (file) => {
  return new Promise((resolve, reject) => {
    ...
    const isSupportedFormat = (filename) => {
      ...
      return /\.(jpe?g|png|gif|bmp|webp)$/.test(name) || name.indexOf(".") === -1;
    };

    if (!isSupportedFormat(file && file.name)) {
      URL.revokeObjectURL(url);
      reject(new Error("Please upload a JPG or PNG"));
      return;
    }

    image.onload = () => {
      // BUG FIX: HEIC / non-decodable images fire onload but report 0 size.
      // Guard against a 0x0 canvas, which makes getImageData throw.
      ...
      const ctx = canvas.getContext("2d", { willReadFrequently: true });
      // BUG FIX: guard against a null context (e.g. browser can't create a 2D context).
      if (!ctx) {
        URL.revokeObjectURL(url);
        reject(new Error("Could not prepare this image for scanning"));
        return;
      }
...
export default function Dashboard() {
  ...
  const [splits, setSplits] = useState(() => {
    try {
      return JSON.parse(localStorage.getItem("expense_splits")) || [];
    } catch {
      return [];
    }
  });
  const [splitForm, setSplitForm] = useState(defaultSplitForm);
...
const request = async (path, options = {}) => {
    ...
    if (res.status === 401 || res.status === 403) {
      // Only treat as a hard logout if we have no token at all, or the
      // response is a genuine auth rejection. Otherwise surface the error.
  ...
  useEffect(() => {
    localStorage.setItem("expense_splits", JSON.stringify(splits));
  }, [splits]);
  ...
  const budgetForecast = useMemo(() => {
    ...
    return Object.entries(budgets).map(([category, budgetAmount]) => {
      const spent = expenses
        .filter((e) => e.category === category && e.date && e.date.startsWith(monthKey))
        .reduce((sum, e) => sum + Number(e.amount || 0), 0);
      const avgDaily = spent / dayOfMonth;
      const projected = avgDaily * daysInMonth;
      const remaining = Number(budgetAmount) - spent;
      const daysLeft = avgDaily > 0 ? Math.max(0, Math.floor(remaining / avgDaily)) : null;
      const pctUsed = Number(budgetAmount) > 0 ? Math.min(999, Math.round((spent / Number(budgetAmount)) * 100)) : 0;
      return { category, budgetAmount: Number(budgetAmount), spent, avgDaily, projected, remaining, daysLeft, pctUsed };
    });
  }, [budgets, expenses]);
  ...
  const netBalances = useMemo(() => {
    ...
    splits.forEach((s) => {
      ...
      map[s.person] = (map[s.person] || 0) + (s.direction === "owes_me" ? amt : -amt);
    });
    return Object.entries(map).map(([person, net]) => ({ person, net }));
  }, [splits]);
...
const runOcr = async (imageSource) => {
    ...
    try {
      ...
      // BUG FIX: if preprocessing fails (HEIC / unsupported / 0x0 image),
      // fall back to running OCR on the raw file instead of crashing.
      fallbackUsed = true;
      processedImage = imageUrl;
    }

    try {
      const rawText = await runOcr(processedImage);
      const parsed = parseReceiptText(rawText);
      const hasAutoFill = Boolean(parsed.title || parsed.amount || parsed.date);
...
setReceiptScan({
        ...
        loading: false,
        error: fallbackUsed
          ? "Please upload a JPG or PNG for best results."
          : rawText.trim()
            ? parsed.amount && parsed.date
              ? ""
              : "Some fields could not be detected. Please fill the blanks manually."
  ...
  const saveBudget = (event) => {
    event.preventDefault();
    if (!budgetDraft.category || !budgetDraft.amount) return;
    setBudgets((current) => ({ ...current, [budgetDraft.category]: Number(budgetDraft.amount) }));
    setBudgetDraft({ ...defaultBudgetDraft, category: budgetDraft.category });
    showToast(`Budget set for ${budgetDraft.category}`);
  };
  ...
  const addSplit = (event) => {
    event.preventDefault();
    if (!splitForm.person || !splitForm.amount) return;
    setSplits((current) => [
      ...current,
      { id: Date.now(), ...splitForm, amount: Number(splitForm.amount), date: today() },
    ]);
    setSplitForm(defaultSplitForm);
    showToast("Split entry added");
  };
  ...
  const startVoiceInput = () => {
    ...
    recognition.onresult = (event) => {
      ...
      setExpenseForm((current) => ({
        ...current,
        title: parsed.description || current.title,
        amount: parsed.amount || current.amount,
        category: parsed.category || current.category,
        source: "voice",
      }));
      showToast("Voice captured — please verify before saving");
    };
  ...
  const addExpense = async (event) => {
    ...
    try {
      ...
      let toastMessage = "Expense added";
      ...
      if (budgetAmount) {
        ...
        const spentBefore = expenses
          .filter((e) => e.category === cat && e.date && e.date.startsWith(monthKey))
          .reduce((sum, e) => sum + Number(e.amount || 0), 0);
        const spentAfter = spentBefore + Number(expenseForm.amount);
        const pct = Math.round((spentAfter / Number(budgetAmount)) * 100);
        if (pct >= 100) {
          toastMessage = `Expense added — you've crossed 100% of your ${cat} budget!`;
          toastType = "error";
        } else if (pct >= 80) {
          toastMessage = `Expense added — ${pct}% of your ${cat} budget used`;
          toastType = "error";
        }
      }

      if (expenseForm.splitWithName && expenseForm.splitAmount) {
        setSplits((current) => [
          ...current,
          {
            id: Date.now(),
            person: expenseForm.splitWithName,
            amount: Number(expenseForm.splitAmount),
            direction: "owes_me",
            note: `Split from "${expenseForm.title}"`,
            date: today(),
          },
        ]);
      }
        ...
        {loading ? (
          ...
          <>
            ...
            {activeTab === "expenses" && (
              <section className="card">
                ...
                <div className="receipt-uploader">
                  ...
                  <div>
                    <h3 style={{ margin: "0 0 8px" }}>Scan receipt or speak</h3>
                    <label className="btn primary" style={{ display: "inline-flex" }}>
                      Upload Receipt
                      <input
                        type="file"
                    ...
                    {voiceSupported && (
                      ...
                      >
                        {voiceListening ? "🎤 Listening..." : "🎤 Speak Expense"}
                      </button>
                    )}
                    {voiceTranscript && (
                      <div className="muted" style={{ marginTop: 8 }}>Heard: "{voiceTranscript}"</div>
                    )}
                    {receiptScan.loading && <span className="verify-badge">Reading receipt... {receiptScan.progress}%</span>}
                    {receiptScan.autoFilled && (
                      <div className="verify-badge">Auto-filled from receipt - please verify</div>
                ...
                <form className="form-row" onSubmit={addExpense} style={{ marginTop: 14 }}>
                  ...
                  <input className="input" placeholder="Split with (name, optional)" value={expenseForm.splitWithName} onChange={(e) => updateExpenseForm("splitWithName", e.target.value)} />
                  <input className="input" type="number" min="0" step="0.01" placeholder="Their share (optional)" value={expenseForm.splitAmount} onChange={(e) => updateExpenseForm("splitAmount", e.target.value)} />
                  <button className="btn primary" type="submit">Add Expense</button>
                </form>
            ...
            {activeTab === "budgets" && (
              <section className="grid-2">
                <div className="card">
                  <h2 className="section-title">Set category budgets</h2>
                  <form className="form-row" onSubmit={saveBudget}>
                    <select className="select" value={budgetDraft.category} onChange={(e) => setBudgetDraft({ ...budgetDraft, category: e.target.value })}>
                      {CATEGORIES.map((item) => <option key={item}>{item}</option>)}
                    </select>
                    <input className="input" type="number" min="0" step="0.01" placeholder="Monthly budget amount" value={budgetDraft.amount} onChange={(e) => setBudgetDraft({ ...budgetDraft, amount: e.target.value })} required />
                  ...
                  {Object.keys(budgets).length ? (
                    <table className="table" style={{ marginTop: 16 }}>
                      <thead>
                        <tr><th>Category</th><th>Budget</th><th>Action</th></tr>
                      </thead>
                      <tbody>
                        {Object.entries(budgets).map(([cat, amt]) => (
                          <tr key={cat}>
                            <td>{cat}</td>
                            <td>{money(amt)}</td>
                            <td><button className="btn danger" onClick={() => removeBudget(cat)}>Remove</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ) : (
                    <div className="empty" style={{ marginTop: 16 }}>No budgets set yet. Add one above to see a forecast.</div>
                  )}
                </div>

                <div className="card">
                  <h2 className="section-title">Forecast — days until budget runs out</h2>
                  {budgetForecast.length ? (
                    <div className="mini-list">
                      {budgetForecast.map((item) => (
                        <div className="mini-item" key={item.category} style={{ flexDirection: "column", alignItems: "stretch", gap: 6 }}>
                          <div style={{ display: "flex", justifyContent: "space-between" }}>
                            <strong>{item.category}</strong>
                            <span className="pill">{item.pctUsed}% used</span>
                          </div>
                          <div className="muted">
                            Spent {money(item.spent)} of {money(item.budgetAmount)} this month
                          </div>
                          <div className="muted">
                            {item.daysLeft === null
                              ? "Not enough data yet — add a few expenses in this category"
                              : item.remaining <= 0
                                ? "Budget already exceeded this month"
                                : `At this rate, you'll run out in ~${item.daysLeft} day${item.daysLeft === 1 ? "" : "s"}`}
                          </div>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="empty">Set a budget on the left to see a forecast here.</div>
                  )}
                </div>
              </section>
            )}
            ...
            {activeTab === "splits" && (
              <section className="grid-2">
                <div className="card">
                  <h2 className="section-title">Add a split entry</h2>
                  <p className="muted">Track shared expenses manually, or use the "Split with" fields on the Add Expense form to create these automatically.</p>
                  <form className="form-row" onSubmit={addSplit}>
                    <input className="input" placeholder="Person's name" value={splitForm.person} onChange={(e) => setSplitForm({ ...splitForm, person: e.target.value })} required />
                    <input className="input" type="number" min="0" step="0.01" placeholder="Amount" value={splitForm.amount} onChange={(e) => setSplitForm({ ...splitForm, amount: e.target.value })} required />
                    <select className="select" value={splitForm.direction} onChange={(e) => setSplitForm({ ...splitForm, direction: e.target.value })}>
                      <option value="owes_me">They owe me</option>
                      <option value="i_owe">I owe them</option>
                    </select>
                    <input className="input" placeholder="Note (optional)" value={splitForm.note} onChange={(e) => setSplitForm({ ...splitForm, note: e.target.value })} />
                    <button className="btn primary" type="submit">Add Split</button>
                  </form>

                  {splits.length ? (
                    <table className="table" style={{ marginTop: 16 }}>
                      <thead>
                        <tr><th>Person</th><th>Amount</th><th>Direction</th><th>Note</th><th>Action</th></tr>
                      </thead>
                      <tbody>
                        {splits.slice().reverse().map((s) => (
                          <tr key={s.id}>
                            ...
                            <td><span className="pill">{s.direction === "owes_me" ? "Owes me" : "I owe"}</span></td>
                            <td>{s.note || "-"}</td>
                            <td><button className="btn danger" onClick={() => removeSplit(s.id)}>Settle</button></td>
                          </tr>
                        ))}
                      </tbody>
                    </table>
                  ) : (
                    <div className="empty" style={{ marginTop: 16 }}>No split entries yet.</div>
                  )}
                </div>
                ...
                <div className="card">
                  ...
                  {netBalances.length ? (
                    <div className="mini-list">
                      {netBalances.map((b) => (
                        <div className="mini-item" key={b.person}>
                          <span>{b.person}</span>
                          <strong style={{ color: b.net >= 0 ? "#22c55e" : "#ef4444" }}>
                            {b.net >= 0 ? `Owes you ${money(b.net)}` : `You owe ${money(Math.abs(b.net))}`}
                          </strong>
                        </div>
                      ))}
                    </div>
                  ) : (
                    <div className="empty">No balances yet.</div>
                  )}
                </div>
              </section>
            )}
